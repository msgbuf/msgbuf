...
class Car_Impl extends Shape_Impl implements Car {
	...
	protected void readField(de.haumacher.msgbuf.json.JsonReader in, String field) throws java.io.IOException {
		switch (field) {
			case WHEEL_1: setWheel1(test.hierarchy.data.Circle.readCircle(in)); break;
			case WHEEL_2: setWheel2(test.hierarchy.data.Circle.readCircle(in)); break;
			case BODY: setBody(test.hierarchy.data.Rectangle.readRectangle(in)); break;
	...
	public static Car readCar_Content(de.haumacher.msgbuf.binary.DataReader in) throws java.io.IOException {
		test.hierarchy.data.Car_Impl result = new Car_Impl();
	...
	protected void readFieldXmlElement(javax.xml.stream.XMLStreamReader in, String localName) throws javax.xml.stream.XMLStreamException {
		switch (localName) {
			case WHEEL_1__XML_ATTR: {
				setWheel1(test.hierarchy.data.Circle_Impl.readCircle_XmlContent(in));
				break;
			}
			case WHEEL_2__XML_ATTR: {
				setWheel2(test.hierarchy.data.Circle_Impl.readCircle_XmlContent(in));
				break;
			}
			case BODY__XML_ATTR: {
				setBody(test.hierarchy.data.Rectangle_Impl.readRectangle_XmlContent(in));
				break;
			}
			default: {
