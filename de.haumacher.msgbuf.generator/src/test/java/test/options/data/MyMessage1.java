...
public interface MyMessage1 extends de.haumacher.msgbuf.data.DataObject, de.haumacher.msgbuf.binary.BinaryDataObject, de.haumacher.msgbuf.observer.Observable, de.haumacher.msgbuf.xml.XmlSerializable {
	...
	/**
	 * Creates a {@link test.options.data.MyMessage1} instance.
	 */
	static test.options.data.MyMessage1 create() {
		return new test.options.data.impl.MyMessage1_Impl();
	}
	...
	/** Reads a new instance from the given reader. */
	static test.options.data.MyMessage1 readMyMessage1(de.haumacher.msgbuf.json.JsonReader in) throws java.io.IOException {
		test.options.data.impl.MyMessage1_Impl result = new test.options.data.impl.MyMessage1_Impl();
		result.readContent(in);
		return result;
	}

	/** Reads a new instance from the given reader. */
	static test.options.data.MyMessage1 readMyMessage1(de.haumacher.msgbuf.binary.DataReader in) throws java.io.IOException {
		in.beginObject();
		test.options.data.MyMessage1 result = test.options.data.impl.MyMessage1_Impl.readMyMessage1_Content(in);
		in.endObject();
		return result;
	}
	...
	public static MyMessage1 readMyMessage1(javax.xml.stream.XMLStreamReader in) throws javax.xml.stream.XMLStreamException {
		in.nextTag();
		return test.options.data.impl.MyMessage1_Impl.readMyMessage1_XmlContent(in);
	}
