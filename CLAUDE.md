...
- **Self-bootstrapping**: Changes to `generator/ast/proto.proto` require regenerating the AST Java classes with an existing generator, then rebuilding. The build does **not** do this: run `java -jar de.haumacher.msgbuf.generator/target/msgbuf-generator-*-full.jar -out de.haumacher.msgbuf.generator/src/main/java de.haumacher.msgbuf.generator/src/main/java/de/haumacher/msgbuf/generator/ast/proto.proto` (or the Eclipse launch config `de.haumacher.msgbuf.generator/bin/Generate Compiler AST.launch`). Likewise, the checked-in generated test sources under `src/test/java/test/**` are regenerated by hand (`-out src/test/java` for each `.proto`).
